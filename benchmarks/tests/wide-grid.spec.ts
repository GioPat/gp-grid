...
import { getBenchmarkColumnCounts, getBenchmarkIterations } from "../src/config/benchmark-config";
  ...
  test(`gp-grid wide layout with ${columnCount.toLocaleString()} columns`, async ({ page }) => {
    ...
    for (const _iteration of Array.from({ length: getBenchmarkIterations() })) {
      ...
      const counts = await page.evaluate(() => ({
        ...
        mountedCells: document.querySelectorAll(".gp-grid-cell").length,
      }));
      ...
      expect(counts.mountedCells).toBe(counts.mountedRows * columnCount);
      samples.push({
        timeToReadyMs,
